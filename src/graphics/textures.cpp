...
GLuint loadTextureArray(const char *imagepath) {
...

  {
    ...
    glTextureSubImage2D(temp_tex, 0, 0, 0, width, height, GL_BGR,
                        GL_UNSIGNED_BYTE, data);

    for (GLsizei i = 0; i < tile_count; ++i) {
      GLint x = (i % tiles_x) * tile_w, y = ((i / tiles_x) * tile_h);
      glCopyImageSubData(temp_tex, GL_TEXTURE_2D, 0, x, y, 0, tileset,
                         GL_TEXTURE_2D_ARRAY, 0, 0, 0, i, tile_w, tile_h, 1);
    }
