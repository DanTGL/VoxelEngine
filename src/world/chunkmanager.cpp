...
ChunkManager::ChunkManager(FrustumG *frustum) {
  ...
  */
  ...
  noise1 = new ImprovedCombinedNoise(new ImprovedOctaveNoise(8, rnd),
                                     new ImprovedOctaveNoise(8, rnd));
  noise2 = new ImprovedCombinedNoise(new ImprovedOctaveNoise(8, rnd),
                                     new ImprovedOctaveNoise(8, rnd));
  noise3 = new ImprovedOctaveNoise(6, rnd);
}

ChunkManager::~ChunkManager() { m_Frustum = NULL; }
...
void ChunkManager::UpdateAsyncChunker() {
  ...
  for (iterator = m_vpChunkAsyncList.begin();
       iterator != m_vpChunkAsyncList.end() &&
       (lNumOfChunksAsynced != ASYNC_NUM_CHUNKS_PER_FRAME);) {
...
void ChunkManager::UpdateLoadList() {
  ...
  std::vector<Chunk *>::iterator iterator;
  for (iterator = m_vpChunkLoadList.begin();
       iterator != m_vpChunkLoadList.end() &&
...
void ChunkManager::UpdateSetupList() {
  ...
  for (iterator = m_vpChunkSetupList.begin();
       iterator != m_vpChunkSetupList.end(); ++iterator) {
    Chunk *pChunk = (*iterator);
    ...
    if (pChunk->IsLoaded() && pChunk->IsSetup() == false) {
      ...
      heightMapBuilder.SetBounds(pChunk->GetX() * Chunk::CHUNK_SIZE,
      (pChunk->GetX() * Chunk::CHUNK_SIZE) + Chunk::CHUNK_SIZE - 1,
      pChunk->GetZ() * Chunk::CHUNK_SIZE, (pChunk->GetZ() * Chunk::CHUNK_SIZE) +
      Chunk::CHUNK_SIZE - 1); heightMapBuilder.Build();*/

      pChunk->Setup();
      // pChunk->SetupLandscape(noise1, noise2, noise3);

      if (pChunk->IsSetup()) {
        // Only force the visibility update if we actually setup the chunk, some
        // chunks wait in the pre-setup stage...
        m_forceVisibilityUpdate = true;
      }
    }
  }
...
void ChunkManager::UpdateRebuildList() {
  ...
  for (iterator = m_vpChunkRebuildList.begin();
       iterator != m_vpChunkRebuildList.end() &&
    ...
    if (pChunk->IsLoaded() && pChunk->IsSetup()) {
      if (lNumRebuiltChunkThisFrame != ASYNC_NUM_CHUNKS_PER_FRAME) {
        ...
        // If we rebuild a chunk, add it to the list of chunks that need their
        // render flags updated since we might now be empty or surrounded.
        m_vpChunkUpdateFlagsList.push_back(pChunk);

        // Also add our neighbours since they might now be surrounded too (If we
        // have neighbours)
...
void ChunkManager::UpdateFlagsList() {
  ...
  for (iterator = m_vpChunkUpdateFlagsList.begin();
       iterator != m_vpChunkUpdateFlagsList.end(); ++iterator) {
    Chunk *pChunk = (*iterator);
...
void ChunkManager::UpdateUnloadList() {
  ...
  for (iterator = m_vpChunkUnloadList.begin();
       iterator != m_vpChunkUnloadList.end(); ++iterator) {
    Chunk *pChunk = (*iterator);
...
void ChunkManager::UpdateVisibilityList(glm::vec3 cameraPosition) {
  ...
  if (m_forceVisibilityUpdate || m_chunkMap.empty() || chunk != lastChunk) {
    ...
    // std::copy(m_vpChunkVisibilityList.begin(), m_vpChunkVisibilityList.end(),
    // std::back_inserter(m_vpChunkUnloadList));
    ...
    /*std::vector<Chunk*>::iterator iterator;
    for (iterator = m_vpChunkUnloadList.begin(); iterator !=
    m_vpChunkUnloadList.end();) { Chunk* pChunk = (*iterator);

            if (pChunk == curChunk || pChunk == pChunkXMinus || pChunk ==
    pChunkXPlus || pChunk == pChunkZMinus || pChunk == pChunkZPlus) { iterator =
    m_vpChunkUnloadList.erase(iterator); continue;
            }
...
void ChunkManager::UpdateRenderList() {
  // Clear the render list each frame BEFORE we do our tests to see what chunks
  // should be rendered
  m_vpChunkRenderList.clear();

  std::vector<Chunk *>::iterator iterator;

  for (iterator = m_vpChunkVisibilityList.begin();
       iterator != m_vpChunkVisibilityList.end(); ++iterator) {
    Chunk *pChunk = (*iterator);
    if (pChunk != NULL) {
      if (pChunk->IsLoaded() && pChunk->IsSetup()) {
        if (pChunk->ShouldRender()) { // Early flags check so we don't always
                                      // have to do the frustum check...
          // Check if this chunk is inside the camera frustum
          // printf("n");
          float c_offset =
              (Chunk::CHUNK_SIZE * BLOCK_RENDER_SIZE) - BLOCK_RENDER_SIZE;
          glm::vec2 chunkCenter =
              pChunk->GetPosition() + glm::vec2(c_offset, c_offset);
          float c_heightoffset =
              (Chunk::WORLD_HEIGHT * BLOCK_RENDER_SIZE) - BLOCK_RENDER_SIZE;
...
void ChunkManager::Render(Shader shader) {
  ...
  for (iterator = m_vpChunkRenderList.begin();
       iterator != m_vpChunkRenderList.end(); ++iterator) {
    Chunk *pChunk = (*iterator);
    if (pChunk->IsLoaded() && pChunk->IsSetup()) {
      if (pChunk->ShouldRender()) { // Early flags check so we don't always have
                                    // to do the frustum check...
...
void ChunkManager::CreateChunk(int x, int z) {
  ...
  heightMapBuilder.SetBounds(
      x * Chunk::CHUNK_SIZE, (x * Chunk::CHUNK_SIZE) + Chunk::CHUNK_SIZE - 1,
      z * Chunk::CHUNK_SIZE, (z * Chunk::CHUNK_SIZE) + Chunk::CHUNK_SIZE - 1);
...
utils::NoiseMap ChunkManager::GetHeightMap() { return m_heightMap; }
...
Chunk *ChunkManager::LoadChunk(glm::ivec2 pos) {
  ...
  heightMapBuilder.SetDestNoiseMap(heightMap);
  heightMapBuilder.SetDestSize(Chunk::CHUNK_SIZE, Chunk::CHUNK_SIZE);
  heightMapBuilder.SetBounds(pos.x, pos.x + Chunk::CHUNK_SIZE - 1, pos.y,
                             pos.y + Chunk::CHUNK_SIZE - 1);
  heightMapBuilder.Build();
